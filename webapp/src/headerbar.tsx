    ...
    renderCore() {
        ...
        return <div id="mainmenu" className={`ui borderless fixed menu ${targetTheme.invertedMenu ? `inverted` : ''} ${manyTutorialSteps ? "thin" : ""}`} role="menubar">
            ...
            <div className="right menu">
                ...
                {showHomeButton && <sui.Item className={`icon openproject ${hasIdentity ? "mobile hide" : ""}`} role="menuitem" title={lf("Início")} icon="home large" ariaLabel={lf("Tela Inicial")} onClick={this.goHome} />}
                {showShareButton && <sui.Item className="icon shareproject mobile hide" role="menuitem" title={lf("Publique seu jogo para criar um link compartilhável")} icon="share alternate large" ariaLabel={lf("Share Project")} onClick={this.showShareDialog} />}
